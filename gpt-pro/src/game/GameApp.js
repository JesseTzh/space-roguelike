...
export class GameApp {
    ...
    rng = new SeededRng(20260529);
    stateMachine = new GameStateMachine('Menu');
    stageSystem = new StageSystem(STAGE_TYPES);
    waveSystem = new WaveSystem();
    controlSystem = new PlayerControlSystem();
    backgroundSystem;
    autoAttackSystem = new PlayerAutoAttackSystem();
    saveSystem = new SaveSystem();
    hud = new Hud();
    playerBulletPool = new ObjectPool(() => new Bullet(), 60, PLAYER_BULLET_LIMIT);
    enemyBulletPool = new ObjectPool(() => new Bullet(), 60, ENEMY_BULLET_LIMIT);
    enemyPool = new ObjectPool(() => new Enemy(), 24, ENEMY_LIMIT);
    explosionPool = new ObjectPool(() => new ExplosionEffect(), 24, EXPLOSION_LIMIT);
    boss = new Boss();
    runState = createRunState();
    player = new PlayerShip({ ...PLAYER_BASE_STATS }, DESIGN_WIDTH / 2, DESIGN_HEIGHT - 160, DESIGN_WIDTH, DESIGN_HEIGHT);
    moduleChoices = [];
    selectedModule;
    installedSelectedModule = false;
    lastTime = 0;
    fps = 60;
    running = false;
    ...
    update(deltaMs) {
        ...
        if (state !== 'Playing' && state !== 'BossStage')
            return;
        ...
        if (this.player.stats.hp <= 0 && !this.player.runtime.dead)
            this.player.runtime.dead = true;
    ...
    handlePlayerInput(deltaSeconds) {
        if (this.uiRoot.children.length > 0)
            return;
        const drag = this.controlSystem.consumeDrag();
        if (drag.x !== 0 || drag.y !== 0)
            this.player.moveBy(drag.x, drag.y);
        const keyboard = this.controlSystem.getKeyboardVector();
        this.player.moveDirection(keyboard.x, keyboard.y, deltaSeconds);
    }
    spawnEnemies() {
        if (this.stageSystem.currentStage.type !== 'normal')
            return;
        const waves = this.waveSystem.getSpawnableWaves(this.stageSystem.elapsed, performance.now(), enemyId => this.enemyPool.allItems.filter(enemy => enemy.active && enemy.configId === enemyId).length);
        for (const wave of waves)
            this.spawnEnemyFromWave(wave);
    }
    ...
    fireBossPattern(pattern, count, damage, speed) {
        if (pattern === 'line') {
    ...
    updateBullets(deltaMs) {
        for (const bullet of this.playerBulletPool.allItems) {
            ...
            if (bullet.x < -50 || bullet.x > DESIGN_WIDTH + 50 || bullet.y < -50 || bullet.y > DESIGN_HEIGHT + 50)
                bullet.despawn();
        }
        ...
        for (const bullet of this.playerBulletPool.allItems) {
            ...
            for (const enemy of this.enemyPool.allItems) {
                if (!enemy.active || enemy.dead)
                    continue;
                if (!circlesOverlap({ x: bullet.x, y: bullet.y, radius: bullet.radius }, { x: enemy.x, y: enemy.y, radius: enemy.radius }))
                    continue;
    ...
    spawnExplosion(x, y, radius = 32) {
        const explosion = this.explosionPool.get();
        if (explosion)
            explosion.spawn(x, y, { radius });
    }
    enterStageResult() {
    ...
    renderStageResult(stageReward, heal) {
    ...
    `;
        for (const button of this.uiRoot.querySelectorAll('[data-module-id]')) {
        ...
        for (const button of this.uiRoot.querySelectorAll('[data-slot-id]')) {
            button.addEventListener('click', () => {
                const slotId = button.dataset.slotId;
                if (!slotId || !this.selectedModule)
                    return;
                const slot = this.runState.shipSlots.find(item => item.id === slotId);
                if (!slot?.unlocked)
                    return;
                if (slot.module && !window.confirm(`替换 ${slot.module.name} 为 ${this.selectedModule.name}？`))
                    return;
        ...
        this.uiRoot.querySelector('[data-action="buy-slot"]')?.addEventListener('click', () => {
            try {
        ...
        this.uiRoot.querySelector('[data-action="next-stage"]')?.addEventListener('click', () => {
            if (!this.installedSelectedModule)
                return;
            this.moduleChoices = [];
            this.selectedModule = undefined;
            this.installedSelectedModule = false;
        ...
        this.uiRoot.querySelector('[data-action="menu"]')?.addEventListener('click', () => this.showMenu());
    }
    enterVictory() {
        if (this.stateMachine.state === 'Victory' || this.stateMachine.state === 'GameResult')
            return;
    ...
    enterDefeat() {
        if (this.stateMachine.state === 'Defeat' || this.stateMachine.state === 'GameResult')
            return;
        this.clearCombatObjects();
        this.boss.despawn();
    ...
    `;
        this.uiRoot.querySelector('[data-action="restart"]')?.addEventListener('click', () => this.startNewRun());
        this.uiRoot.querySelector('[data-action="menu"]')?.addEventListener('click', () => this.showMenu());
    }
    togglePause() {
        const state = this.stateMachine.state;
    ...
    `;
        this.uiRoot.querySelector('[data-action="resume"]')?.addEventListener('click', () => this.togglePause());
        this.uiRoot.querySelector('[data-action="menu"]')?.addEventListener('click', () => this.showMenu());
    }
    showMenu() {
        this.clearCombatObjects();
    ...
    `;
        this.uiRoot.querySelector('[data-action="start"]')?.addEventListener('click', () => this.startNewRun());
        for (const input of this.uiRoot.querySelectorAll('[data-setting]')) {
            input.addEventListener('change', () => {
                const data = this.saveSystem.load();
                if (input.dataset.setting === 'music')
                    data.settings.musicEnabled = input.checked;
                if (input.dataset.setting === 'sfx')
                    data.settings.sfxEnabled = input.checked;
                if (input.dataset.setting === 'vibration')
                    data.settings.vibrationEnabled = input.checked;
    ...
    drawImageOrShape(key, x, y, width, height, fallbackColor) {
    ...
    drawSmallHpBar(x, y, width, ratio) {
        this.ctx.save();
    ...
    installTestHooksIfNeeded() {
        const isE2E = new URLSearchParams(window.location.search).has('e2e') || Boolean(import.meta.env?.MODE === 'e2e');
        if (!isE2E)
            return;
        const hooks = {
