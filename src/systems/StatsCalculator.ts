...
export function applyModuleEffect(stats: PlayerStats, effect: ShipModule['effects'][number]): void {
  switch (effect.type) {
    case 'damage_percent':
    ...
    case 'on_death_revive_once':
      break
    default: {
      const unknown: never = effect.type
...
export function calculateFinalStats(baseStats: PlayerStats, modules: ShipModule[], currentHp: number): PlayerStats {
